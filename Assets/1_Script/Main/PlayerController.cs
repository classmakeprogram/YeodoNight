...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Mouse Look (시야 조절)")]
    public float mouseSensitivity = 150f; // 마우스 감도
    private float xRotation = 0f;         // 위아래 회전 값 저장
    ...
    public float bulletSpeed = 40f;      // 발사되는 총알 속도
    ...
    void Update()
    {
        HandleMouseLook();   // 마우스 시야 회전 처리
        HandleMovement();    // 키보드 이동 처리
        HandleWeaponSwap();  // 무기 교체 처리
        HandleCombat();      // 전투 및 발사 처리
        UpdateUI();          // UI 업데이트
    }
    ...
    void HandleMouseLook()
    {
        ...
        xRotation -= mouseY;
        xRotation = Mathf.Clamp(xRotation, -85f, 85f); // 위아래 회전 제한
    ...
    private void ShootAK47()
    {
        ...
        if (Physics.Raycast(playerCam.transform.position, playerCam.transform.forward, out hit, range))
        {
            ...
            // [에러 방지 적용] CompareTag 대신 .tag 문자열 비교를 사용하여 태그가 프로젝트에 없어도 뻗지 않도록 함
